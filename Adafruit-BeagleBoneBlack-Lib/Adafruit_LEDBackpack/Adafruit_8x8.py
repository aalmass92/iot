...
class EightByEight(LEDBackpack):
  ...
  def drawPixel(self, x, y, color=1):
    ...
    if (color):
      self.setBufferRow(y, buffer[y] | 1 << x )    # disable update option to determine if writeDisplay() is called when 
    else:                                                # writing a bunch of pixels. Speeds things up dramatically when scrolling 
      self.setBufferRow(y, buffer[y] & ~(1 << x)) # text on multiple matrices. Call writeDisplay() after updating buffer.
